...
assert.ok(
  model.seed
    .slice(29)
    .every((s) => s.level === null && s.interest === 0 && !s.pinned && !s.lastUsed),
);
assert.ok(
  model.matchesQuery(
    model.seed.find((s) => s.id === 'photoshop'),
    'フォトショ',
  ),
);
assert.ok(
  model.matchesQuery(
    model.seed.find((s) => s.id === 'sheets'),
    'スプシ',
  ),
);
assert.ok(
  model.matchesQuery(
    model.seed.find((s) => s.id === 'react'),
    'ｒｅａｃｔ',
  ),
);
...
for (const skill of model.seed) {
  ...
  for (const id of skill.related)
    assert.ok(ids.has(id), `${skill.id} references missing skill ${id}`);
}
assert.equal(model.loadSkills().find((s) => s.id === 'freertos').level, 3);
localStorage.setItem(
  model.storageKey,
  JSON.stringify([{ id: 'freertos', level: 4, interest: 0, pinned: false }]),
);
...
localStorage.setItem(
  model.storageKey,
  JSON.stringify([{ id: 'freertos', level: 99, interest: -1 }]),
);
assert.equal(model.loadSkills().find((s) => s.id === 'freertos').level, 3);
...
assert.ok(
  model.sector(112, 226, 0.5, 359.5).includes('0 1 1'),
  'Focused category must render a large arc',
);
const { default: MapExplorer } = await compile('src/MapExplorer.tsx');
const mapHtml = renderToString(
  createElement(MapExplorer, {
    skills: model.seed,
    selected: model.seed.find((s) => s.id === 'freertos'),
    select: () => {},
    query: '',
...
values.clear();
const { default: AllSkillsGraph, relationEdges } = await compile('src/AllSkillsGraph.tsx');
assert.deepEqual(
  relationEdges([
    { id: 'a', related: ['b', 'a', 'missing'] },
    { id: 'b', related: ['a'] },
  ]),
  [['a', 'b']],
  'Relations must deduplicate reciprocal links and omit invalid targets',
);
const graphHtml = renderToString(
  createElement(AllSkillsGraph, {
    skills: model.seed,
    selected: model.seed[0],
    select: () => {},
    query: '',
  }),
);
assert.equal(
  (graphHtml.match(/class="all-graph-node"/g) ?? []).length,
  model.seed.length,
  'Every skill must be present in the overview',
);
assert.equal(
  (graphHtml.match(/<line /g) ?? []).length,
  relationEdges(model.seed).length,
  'Every relation must be drawn once',
);
assert.ok(!graphHtml.includes('NaN'));
const { default: App } = await compile('src/App.tsx');
const html = renderToString(createElement(App));
for (const label of ['SkillTopo', 'スキルを読み込んでいます'])
  assert.ok(html.includes(label), `Initial render missing ${label}`);
...
assert.ok(
  blank.skills.every(
    (s) => s.level === null && s.interest === 0 && !s.pinned && !s.notes && !s.lastUsed,
  ),
);
const sparse = {
  ...blank,
  userName: 'サンプル A',
  skills: [{ ...blank.skills[0], level: 4, notes: '実務で使用' }],
};
const imported = persistence.parseProfile(JSON.stringify(sparse));
const binary = await persistence.encodeProfileBytes(sparse, 'binary');
assert.deepEqual(await persistence.decodeProfileBytes(binary), imported);
assert.deepEqual(JSON.parse(gunzipSync(binary.slice(8)).toString()), sparse);
assert.deepEqual(
  await persistence.decodeProfileBytes(await persistence.encodeProfileBytes(sparse, 'json')),
  imported,
);
const fixture = readFileSync('../examples/tanaka.skilltopo');
assert.deepEqual(
  await persistence.decodeProfileBytes(fixture),
  persistence.parseProfile(readFileSync('../examples/tanaka.json', 'utf8')),
);
for (const length of [7, 8, 12, binary.length - 1])
  await assert.rejects(persistence.decodeProfileBytes(binary.slice(0, length)));
const future = binary.slice();
future[7] = 2;
await assert.rejects(persistence.decodeProfileBytes(future), /バージョン/);
const corrupt = binary.slice();
corrupt[corrupt.length - 8] ^= 1;
await assert.rejects(persistence.decodeProfileBytes(corrupt), /破損/);
const oversized = Buffer.concat([
  binary.slice(0, 8),
  gzipSync(Buffer.alloc(8 * 1024 * 1024 + 1, 32)),
]);
await assert.rejects(persistence.decodeProfileBytes(oversized), /8 MB/);
...
for (const invalid of [
  ...
  { ...sparse, userName: '' },
  { ...sparse, skills: [] },
  { ...sparse, skills: [{ ...sparse.skills[0], level: 5 }] },
  { ...sparse, skills: [{ ...sparse.skills[0], related: ['missing'] }] },
]) {
...
await firstSave;
assert.equal(
  calls[1].args.profile.userName,
  blank.userName,
  'Save must capture a profile snapshot',
);
assert.equal(calls[1].args.profile.skills[0].level, null);
assert.equal(calls[1].args.saveAs, true);
assert.equal(calls[1].args.fileFormat, 'binary');
window.__TAURI__.core.invoke = async () => null;
assert.equal(
  await persistence.openProfile(false),
  null,
  'Cancelling a picker leaves the current document intact',
);
...
await assert.rejects(
  persistence.openProfile(false),
  /disk full/,
  'Desktop errors must not silently fall back to browser data',
);
console.log(
  `PASS: ${model.seed.length} skills; startup render; profile validation; sparse import; native IPC; save snapshots; cancellation and error propagation.`,
);
